...
class OrAndULSHRecommender(LSHRecommender):
    """
...
if __name__ == '__main__':
    def tune_param(ratio = 0.1, times = 50):
        ...
        for t in range(times):
            ...
            for (t_idx, table_num) in enumerate(num_of_hash_tables):
                for (f_idx, function_num) in enumerate(num_of_hash_functions):
                    recommender = OrAndULSHRecommender(raw_data, data, num_of_tables=table_num,
        ...
        f = h5py.File('../../output/basic_ulsh.h5py', 'a')
        ...
        return np.argmin(maes), np.argmin(rmses)
