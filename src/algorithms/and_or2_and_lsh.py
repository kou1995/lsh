...
class AndOr2AndULSHRecommender(LSHRecommender):
    """
    ...
    cascading AND-OR-OR-AND construction together.
    ...
    def __init__(self, raw_data, data, num_of_functions = 4, num_of_tables = 4, seed = 1):
        ...
        self.similarity_matrix = np.ones((self.num_of_users, self.num_of_users))
    ...
    def classify(self):
        ...
        for i in range(self.num_of_tables):
            ...
            mat2 = np.zeros((self.num_of_users, self.num_of_users))
        ...
        self.similarity_matrix = (self.similarity_matrix > 0).astype(np.float)
...
if __name__ == '__main__':
    def tune_param(ratio = 0.1, times = 50):
        ...
        num_of_hash_functions = [12, 16, 18, 20]
        num_of_hash_tables = [14, 12, 10, 8]
        ...
        for t in range(times):
            ...
            for (t_idx, table_num) in enumerate(num_of_hash_tables):
                for (f_idx, function_num) in enumerate(num_of_hash_functions):
                    recommender = AndOr2AndULSHRecommender(raw_data, data, num_of_tables=table_num,
        ...
        f = h5py.File('../../output/and_or_or_and_ulsh_more_function.h5py', 'a')
        f.create_dataset('maes', data=maes)
        f.create_dataset('rmses', data=rmses)
        f.create_dataset('num_of_similars', data=num_of_similars)
