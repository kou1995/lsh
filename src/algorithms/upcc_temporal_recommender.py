...
class UPCCTemporalRecommender():
    def __init__(self, raw_data, data, last_slot = 63, top=30):
        ...
        self.data_for_calc = data.reshape(self.num_of_users, self.num_of_services * self.num_of_slices)
        self.top = top
    ...
    def classify(self):
        ...
        :return:
        """
        user_average_data = np.average(self.data_for_calc, axis=1).reshape(self.num_of_users, 1)
        variance = self.data_for_calc - user_average_data
    ...
    def classify_cosine(self):
        ...
        :return:
        """
        norms = np.linalg.norm(self.data_for_calc, axis=1)
        norms = norms.reshape(self.data_for_calc.shape[0], 1)
        norms_matrix = np.dot(norms, norms.T)

        dot_matrix = np.dot(self.data_for_calc, self.data_for_calc.T)
    ...
    def calculateSimilarity(self, u1, u2):
        ...
        user1 = self.data_for_calc[u1, :]
        user2 = self.data_for_calc[u2, :]
...
if __name__ == '__main__':
    def tune_param(ratio = 0.1, times = 50):
        ...
        recommender = UPCCTemporalRecommender(raw_data, data,  top=5)
